...
- select和poll并没有本质的区别，都使用过一个**线性结构**来存储关注的socket集合（select 使用固定长度的 BitsMap，poll用了一个动态数组，但是以链表形式组织），**时间复杂度为 O(n)，而且也需要在用户态与内核态之间拷贝文件描述符集合**
    1. 把关注的socket集合通过select/poll系统调用从**用户态拷贝到内核态**
    2. 然后内核去检测有没有事件，有事件的时候内核就**遍历关注这个集合**，找到对应的socket，并设置其状态为可读或者可写
    3. 然后又把整个socket集合从**内核态拷贝到用户态**
    4. 用户态还要**遍历整个socket集合**找到这个可读或者可写的socket，在对其处理，所以如果socket集合越来越大，它的遍历和拷贝都会带来很大开销
- 而epoll解决了上述两个的问题(使用**红黑树**+**双链表**存储)
    1. 只需要在`epoll_ctl`函数（control的缩写），也就是建立连接的时候传递一次文件描述符，`epoll_wait`函数就可以不用传递了
    2. 内核中使用**红黑树**关注所有待检测的socket，这个数据结构很高效，增删改时间复杂度一半是logn，如果有新的socket加入直接往里面复制这个socket就行，不用拷贝整个socket集合
    3. epoll 使用**事件驱动**的机制，内核里**维护了一个链表来记录就绪事件**，当某个 socket 有事件发生时，通过**回调函数**内核会将其加入到这个就绪事件列表中，当它只会把有事件发生的socket集合传递给应用程序
    ...
    > **什么时候select比epoll好？**
    >
    > 在**少连接高并发**的情况下。因为，连接少意味着不会超过select  **1024**的上限，高并发意味着一次wait，每一个连接都会来数据。把扫描有事件连接时的O(n)的复杂度降至O(1)。
    >
    > select的差距主要体现在每次内核O(n)的去遍历fd，用户也需要去遍历fd，造成效率低下
    >
    > **同步、异步、阻塞、非阻塞**
    >
    > select,poll,epoll本质上都是同步非阻塞I/O,因为他们都需要在读写事件就绪后自己负责进行读写,而异步I/O则无需自己负责进行读写,内核来完成
...
1. **端口数限制**
   - 本地可用的临时端口数量有限（通常 32768–65535，大概 28K 个）。
   - 如果同一个客户端 IP 去连同一个服务端 IP:Port，那最多只有这么多个连接。
2. **文件描述符限制（OS 层面）**
   - 每个 TCP 连接都要占用一个 socket fd。
   - Linux 里 `ulimit -n`、`/proc/sys/fs/file-max` 会限制最大打开文件数。
3. **内核资源限制**
   - 每个连接需要 TCP 控制块（TCB）、发送/接收缓冲区。
   - 如果内存不足，也会限制连接数。
4. **网络栈参数**
   - 如 `net.ipv4.ip_local_port_range`、`somaxconn`、`tcp_max_syn_backlog` 这些参数都会影响可同时建立的连接数。
...
6. 最终的数据包生成之后，网卡将其复制到网卡的缓冲区中，再给数据包开头加上报头和起始帧分节符，末尾加上检验错误的帧检验序列
7. 网卡将最终的数据包转化为电信号，通过网线发送出去
8. 遇到交换机，查看交换机的MAC地址表，有没有记录目标MAC地址可以走哪个端口，找到了就直接发送到这个端口，找不到就直接群发（除了源端口）
9. 遇到路由器，根据路由表查询转发目标，再由相对应的端口作为发送发将以太网包发送出去

10. 当数据包到达服务器网卡的时候，网卡写入DMA区域（ring buffer)

11. 内核网络栈开始处理数据包--> 取出MAC头，验证MAC地址；取出IP头，检查IP是否符合，是否是TCP；取出TCP头，查看序列号等；把数据放入接受缓存区

12. 服务器接收到数据包后，将请求的网页内存装到HTTP响应报文中，发送出去

13. 浏览器收到HTTP报文后，就可以解析找个HTML文件成DOM树结构了，然后执行渲染。
...
1. 使用**DNS服务**解析域名，找到对应的IP地址
2. 主机构建一个 **ICMP 格式的数据包**，由 ICMP 协议将这个数据包（有效负荷）连同目的 IP 地址一起交给 IP 协议，IP协议生成**IP头部**给数据包加上
3. **生成MAC地址**给数据包加上（发送方的MAC地址如果没有缓存，则通过ARP发送广播获取）
4. 最终的数据包生成之后，**网卡将其复制到网卡的缓冲区**中，再给数据包开头加上报头和起始帧分节符，末尾加上检验错误的帧检验序列
5. 网卡将最最最终的**数据包转化为电信号**，通过网线发送出去

#### linux如何收发数据包

- 接收流程：
    1. 当有网络包到达时，会通过** DMA 技术**，将网络包**写入到指定的内存地址**，接着网卡向** CPU **发起**硬件中断**，当 CPU 收到硬件中断请求后，根据**中断表**，调用已经注册的**中断处理函数**。
    2. 硬件中断处理函数:
        - 需要先**「暂时屏蔽中断」**
        - 接着，发起**「软中断」**，然后恢复刚才屏蔽的中断。
    3. 内核中的 **ksoftirqd 线程**专门负责软中断的处理，当 ksoftirqd 内核线程收到软中断后，就会来**轮询**处理数据。

        ksoftirqd 线程会从 Ring Buffer 中获取一个**数据帧**，用 sk_buff 表示，从而可以作为一个网络包交给网络协议栈进行逐层处理。
    4. 然后网络协议栈就会分层拆包装
- 发送流程：
    1. 应用程序**调用Socket**发送数据包的接口，Socket层会将应用层数据**拷贝**到**Socket发送缓冲区**
    2. 接下来**网络协议栈**按照 TCP/IP 协议栈从上到下逐层处理。
    3. 最后触发**软中断**去告诉**网卡驱动程序**，网卡驱动程序会通过**DMA**从**发包队列**中读取网络包，将其放入硬件网卡的队列中，随后物理网卡再将其发送出去
...
●服务端收到客户端的FIN报文时，先回一个ACK应答报文，而服务端可能还有数据需要处理和发
送，等服务端不再发送数据时，才发送FIN报文给客户端来表示同意现在关闭连接。
从上面过程可知，服务端通常需要等待完成数据的发送和处理，所以服务端的ACK和FIN一般都会分
开发送，因此是需要四次挥手。
...
GET请求是幂等的，因为它是「只读」操作，无论操作多少次每次的结果都是相通的。POST是因为「新增或提交」数据的操作会修改服务器上的资源，所以不是幂等的。
...
向服务端发送一个生成的随机数R1(random1),当前浏览器的TLS版本，以及加密方式

2. Server Hello

服务器的话会返回一个随机数R2(random2),确认支持的TLS版本，确认加密方法。然后还会发送数字证书（包含了公钥）

3. client

收到服务器的消息后，会取出证书中的公钥，然后随机生成一个R3(也叫做pre-master)，然后使用公钥加密这个R3，发给服务器。在client这边就能通过R1,R2,R3计算出会话密钥，握手后后续的通信都会用这个会话密钥加密

4. server

服务端用自己的私钥解密出R3,然后同样可以用R1,R2,R3计算出会话密钥。接下去就可以使用会话密钥了。
