...
1） 区分左值和右值的方法主要是看能不能对表达式取地址，如果可以是左值，否则是右值。
...
在了解了右值后，再来说右值引用，`T&& k = getVar()`.左值引用时对左值的引用,同理右值引用就是对右值的引用(右值是匿名的变量,我们也只能通过右值引用的方式获得右值).右值引用让右值的生命周期变长了.

```c++
T&& k = getVar() //表达式产生的临时变量不会随着表达式结束被销毁,它的生命周期与右值引用类型变量的生命周期一样长
T k = getVar()
```

右值引用还有一个特点是`int&& i = 5`,这个i的类型是右值引用,但是`i`本身是一个左值.

所以T&& t在发生**自动类型推断**的时候，它是未定的引用类型（universal references），如果被一个左值初始化，它就是一个左值；如果它被一个右值初始化，它就是一个右值，它是左值还是右值取决于它的初始化。

对于类中的移动构造函数,他的参数会是一个右值引用,这样的话对于带有堆内存的类,有时候就不一定需要深拷贝了,可以直接通过移动构造函数,把内存的所有权进行转移.
...
LRU是缓存淘汰策略，按照「最近最少使用的原则淘汰」。

LRU缓存的实现考虑以下需求：

+ 要能快速访问到数据,于是可以用哈希表提供O(1)的随机访问
+ 要能够维护数据的最近使用顺序，以便淘汰。

-->于是一个LRU缓存应该有：

+ 使用哈希表提供 O(1) 时间复杂度的快速访问能力。
+ 使用双向链表记录访问顺序，支持在 O(1) 时间复杂度内更新使用顺序
+ 当缓存容量超出限制时，删除链表尾对应的元素
