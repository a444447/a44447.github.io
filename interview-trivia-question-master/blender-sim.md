...
1. Log是Leader用来对操作排序的一种手段。对于运行在raft层之上的状态机来说，对于这些复制状态机来说，所有副本不仅要执行相同的操作，还需要用**相同的顺序**执行这些操作。而Log与其他很多事物，共同构成了Leader对接收到的客户端操作分配顺序的机制
...
> 📌 **面试点**：“为什么不能直接加为 Voter？” —— 因为它可能还没同步，加入投票会导致日志不一致或分裂投票。
...
双向通信通道，然后一个Channel内部是由两个共享内存区域组成的，每个共享内存承担一个方向的数据传输，比如第一个共享内存用于从源进程写数据，目标进程读数据；第二个共享内存区域则相反。
...
#### SCB 是如何启动 Blender 子进程的？命令行参数如何动态生成？
...
当前系统在初版实现时确实还没有对 Blender 崩溃后的自动重启机制，但我在调试过程中意识到这个问题，也在评估引入一个轻量的健康检查+自动恢复机制。

我的设计方案大致如下：

1. **SCB 状态标记 + 父进程监控：**
    每个 Blender 进程对应一个 `SCB` 对象，我们可以在 `SCBControler` 中周期性地通过 `waitpid(pid, WNOHANG)` 或 `kill(pid, 0)` 检查其存活状态。
2. **故障恢复：**
    一旦检测到某个 SCB 所属进程退出，可以将其状态设为 `ERROR`，然后调用已有的构造函数 + `fork` + `execvp` 重新启动该 Blender。
3. **配合任务系统：**
    重启完成后会获得一个新 PID，对应的通信通道和任务表会自动更新，系统即可恢复服务。
...
### 任务执行过程中如何记录日志？有没有集中式监控方案？

当前系统中我们使用了自研的轻量级日志模块（`Logger.h`），支持日志等级（Debug/Info/Error）以及线程安全输出，基本可以满足本地调试和运行时定位问题的需求。

因为我们的 Logger 模块是自己写的，底层用的是 `std::cerr`，所以改成输出到文件或结构化格式也很容易。
