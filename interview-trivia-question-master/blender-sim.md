...
首先raft的默认模型中，读请求也需要交给leader来处理，也就是保证线性一致的读。

| 要保证的性质                                                | 如果让 Follower 单独回复会发生什么？                         |
| ----------------------------------------------------------- | ------------------------------------------------------------ |
| **最新性**：读取一定≥所有已提交写                           | Follower 可能落后于最新 `commitIndex`，“脏读”                |
| **单调顺序**：A 写先完成，B 读随后发起，B 必须看到 A 的结果 | Follower 与 Leader 之间没有全序日志保证，容易读到 **A 之前** 的状态 |
| **对外一致的时间线**                                        | 多个 Follower 并行服务读会暴露“谁更新、谁没更”的混乱视图     |

有一些改进的操作来绕过leader

+ ReadIndex，线性一致性的follower读，Follower 收到读→向 Leader 发一次 **ReadIndex 流程**（或 `LeaseCheck`）→确认自己日志追到 `readIndex` 后本地返回。
...
*注意我们的心跳是entries=[]的appendEntries RPC，不会添加日志。*

首先我们要清楚，获得多数选票只能说明候选人的日志不比投票者旧，但是不保证投票者把候选人的最后一条日志写入磁盘。因此当选后，进行一次no-op，是为了把leader的日志都提交。
